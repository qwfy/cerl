...
class CERL_Agent:
    ...
    def __init__(self, args):
        ...
        for _ in range(args.pop_size):
            ...
            if ALGO == 'SAC':
                self.pop.append(GaussianPolicy(args.state_dim, args.action_dim, args.hidden_size, wwid))
            else:
                self.pop.append(Actor(args.state_dim, args.action_dim, wwid))

        if ALGO == "SAC":
            self.best_policy = GaussianPolicy(args.state_dim, args.action_dim, args.hidden_size, -1)
        else:
        ...
        for _ in range(len(self.portfolio)):
            if ALGO == 'SAC':
                self.rollout_bucket.append(GaussianPolicy(args.state_dim, args.action_dim, args.hidden_size, -1))
            else:
                self.rollout_bucket.append(Actor(args.state_dim, args.action_dim, -1))
        ...
        self.evo_workers = [Process(
            target=rollout_worker,
            args=(
            id, self.evo_task_pipes[id][1], self.evo_result_pipes[id][0], False, self.data_bucket, self.pop, ENV_NAME,
            None, ALGO)
        ...
        self.workers = [Process(
            target=rollout_worker,
            args=(
            id, self.task_pipes[id][1], self.result_pipes[id][0], True, self.data_bucket, self.rollout_bucket, ENV_NAME,
            args.noise_std, ALGO)
        ...
        self.test_workers = [Process(
            target=rollout_worker,
            args=(
            id, self.test_task_pipes[id][1], self.test_result_pipes[id][0], False, None, self.test_bucket, ENV_NAME,
            None, ALGO)
        ...
        for i in range(args.rollout_size): self.allocation.append(
            i % len(self.portfolio)
            )  # Start uniformly (equal resources)
    ...
    def train(self, gen, frame_tracker):
        ...
        if self.replay_buffer.__len__() > self.args.batch_size * 10:  ###BURN IN PERIOD
            ...
            threads = [threading.Thread(
                ...
                args=(self.replay_buffer, self.args.buffer_gpu, self.args.batch_size,
...
if __name__ == "__main__":
    ...
    frame_tracker = utils.Tracker(
        ...
        '.csv'
        )  # Tracker class to log progress
    max_tracker = utils.Tracker(
        args.aux_folder,
        ['pop_max_score_' + ENV_NAME + SAVETAG],
        '.csv'
        )  # Tracker class to log progress FOR MAX (NOT REPORTED)
